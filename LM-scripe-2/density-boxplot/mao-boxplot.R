...
# #标准化后数据箱线图
# bb<-melt(nor2)
# #bb$variable<-factor(bb$variable,levels=unique(bb[,1]))
# variablesss<-factor(bb$variable,levels=unique(bb[,1]))
# pp<-ggplot(bb,aes(x=variable,y=value,color=variablesss))+
#   stat_boxplot(geom = "errorbar",width=0.35)+
#   geom_boxplot(aes(fill=variable))+
#   geom_boxplot(alpha=0.5)+
#   labs(x="",y="Normalized Protein Expression") +
#   #theme(legend.position="top")+
#   scale_fill_discrete(guide = FALSE)+
#   guides(color=guide_legend(title=NULL))+
#   theme(legend.text=element_text(size=15))+
